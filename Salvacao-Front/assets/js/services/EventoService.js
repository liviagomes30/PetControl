...
class EventoService {
  constructor() {
    this.baseURL = "http://localhost:8080/evento"; // CORREÇÃO: Usar a URL completa do backend.
  }

  // Método para listar todos os eventos
  async listarTodos() {
    try {
      // A requisição agora será para http://localhost:8080/evento/listar
      ...
      if (!response.ok) {
        if (response.status === 404) {
          return []; // Retornar um array vazio se a rota não for encontrada ou não houver eventos
        }
  ...
  async listarAnimais() {
    try {
      const response = await fetch("http://localhost:8080/animais", {
        // CORREÇÃO: Usar a URL completa do backend.
        method: "GET",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  async atualizarStatus(id, status) {
    try {
      ...
      console.error(`Erro ao atualizar status do evento ${id}:`, error);
      throw error;
    }
  }
}
